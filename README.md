...
A local-first desktop application for managing and running **Excalidraw** and **tldraw** canvases, organised into projects. All data is stored on-device in a SQLite database — nothing leaves your machine.
...
- **Project organisation** — group canvases into named projects; inline rename any project or canvas
- **Auto-save** — changes are debounced and written to SQLite automatically; no save button needed
...
- **Canvas operations** — duplicate a canvas (including its data), move a canvas to a different project
- **Recency sorting** — canvases are ordered by last-opened time so the most recent work surfaces first
- **Keyboard shortcuts** — `Escape` to navigate back / close modals
...
│   │   ├── CanvasGrid.tsx          # Canvas cards + create modal + duplicate / move / delete
│   │   ├── CanvasEditor.tsx        # Full-screen editor shell + breadcrumb + save status
│   │   └── editors/
│   │       ├── ExcalidrawEditor.tsx  # Loaded via dynamic(ssr:false)
│   │       └── TldrawEditor.tsx      # Loaded via dynamic(ssr:false)
│   └── lib/
│       ├── types.ts                # Project + Canvas TypeScript interfaces
...
After each successful write the header shows **Saved ✓** for 2 seconds.
...
Canvas editors produce a stream of micro-changes. Debouncing at 500 ms collapses rapid edits into a single write, keeping SQLite load low while ensuring the last state is always persisted before the user navigates away.
